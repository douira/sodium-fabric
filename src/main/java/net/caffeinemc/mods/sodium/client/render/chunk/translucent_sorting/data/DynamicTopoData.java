...
public class DynamicTopoData extends DynamicData {
    ...
    private void updateHeuristicType() {
        this.heuristicType = deriveHeuristicType(this.GFNITrigger, this.directTrigger);
    }

    private static HeuristicType deriveHeuristicType(boolean GFNITrigger, boolean directTrigger) {
        if (GFNITrigger) {
            return directTrigger ? HeuristicType.DYNAMIC_DISTANCE_TOPO : HeuristicType.DYNAMIC_TOPO;
        } else if (directTrigger) {
            return HeuristicType.DYNAMIC_DISTANCE;
        } else {
            throw new IllegalStateException("No trigger enabled");
        }
    }
    ...
    public class DynamicTopoSorter extends DynamicSorter implements IntConsumer {
        ...
        void writeSort(CombinedCameraPos cameraPos, boolean initial) {
            ...
            if (this.GFNITrigger && !this.isDirectTrigger) {
                ...
                topoSortRecorder.recordNow(DynamicTopoData.this.getQuadCount(), sortStart);
                this.intBuffer = null;
            ...
            if (this.directTrigger) {
                ...
                distanceSortRecorder.recordNow(DynamicTopoData.this.getQuadCount(), start);
            }
